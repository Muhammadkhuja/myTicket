...
export class CustomercardService {
  constructor(
  @InjectModel(CustomerCard.name) private customecardSchema:  Model<CustomerCard>,
  ...
  update(id: string, updateCustomercardDto: UpdateCustomercardDto) {
    const updates = this.customecardSchema.findByIdAndUpdate(id, updateCustomercardDto)
    if(!updates){
      throw new NotFoundException("Custome card topilmadi")
    }
  }
